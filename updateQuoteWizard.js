...
const newQuoteModalHTML = `    <div id="quote-modal" class="fixed inset-0 bg-dark/95 backdrop-blur-xl z-[100] hidden flex-col items-center justify-center">
        <div class="glass rounded-3xl p-6 md:p-8 max-w-2xl w-full mx-6 relative border-white/10 mt-10 md:mt-0 max-h-[90vh] overflow-y-auto custom-scrollbar">
            ...
            <form id="quote-form" class="w-full">
                ...
                <!-- STEP 1: Layout -->
                <div id="quote-step-1" class="step-content block animate-fade-up">
...
// The new JS logic
const newQuoteLogicJS = `    // ─── Quote Modal Logic (Wizard) ───
    ...
    function goToQuoteStep(step) {
        ...
        if(step > 1 && currentQuoteStep === 1) {
            ...
            // Auto-calc
            const floorplan = form.querySelector('input[name="Floorplan"]:checked').value;
            let sqftInput = document.getElementById('quote-sqft');
            if(!sqftInput.value || sqftInput.dataset.auto === "true") {
                let beds = 1;
                if (floorplan === "2 BHK") beds = 2;
                else if (floorplan === "3 BHK") beds = 3;
                else if (floorplan === "4+ BHK") beds = 4;
                sqftInput.value = (beds * 70) + 70;
                sqftInput.dataset.auto = "true";
            }
        }
        
        if(step > 2 && currentQuoteStep === 2) {
            const sqft = parseInt(document.getElementById('quote-sqft').value);
            if(!sqft || sqft <= 0) return alert("Please enter a valid square footage.");
            document.getElementById('quote-sqft').dataset.auto = "false";
        }
        
        if(step > 3 && currentQuoteStep === 3) {
            if(!form.querySelector('input[name="Package"]:checked')) {
                return alert("Please select a Package to continue.");
            }
            
            // Calculate final
            const sqft = parseInt(document.getElementById('quote-sqft').value);
            const packageEl = form.querySelector('input[name="Package"]:checked');
            const rate = parseInt(packageEl.dataset.rate);
            const packageName = packageEl.value;
            ...
            document.getElementById('quote-math-breakdown').textContent = '(' + sqft + ' sqft × ₹' + rate + ' / sqft ' + packageName + ')';
            document.getElementById('hidden-estimate').value = formattedEstimate + ' (' + sqft + ' sqft, ' + packageName + ' package)';
        }
...
// Replace HTML
const startHTML = content.indexOf('<div id="quote-modal"');
const endHTML = content.indexOf('<!-- ═══════════════════════════════════════════\r\n         REFERRAL MODAL');
if (startHTML !== -1 && endHTML !== -1) {
    content = content.substring(0, startHTML) + newQuoteModalHTML + '\n\n\n    ' + content.substring(endHTML);
}

// Replace JS Logic
const startJS = content.indexOf('    // ─── Quote Modal Logic ───');
const endJS = content.indexOf('document.getElementById(\'quote-form\').addEventListener(\'submit\', async function(e) {');
if (startJS !== -1 && endJS !== -1) {
    const endJSBlock = content.indexOf('        try {', endJS) + 13;
    content = content.substring(0, startJS) + newQuoteLogicJS + content.substring(endJSBlock);
}

fs.writeFileSync(file, content);
console.log('Wizard UI successfully written to index.html!');
